...
function getTealWithoutVersion(inputTeal: TEALInfo[]) {
  const teal = inputTeal
    .map((t) => t.teal)
    .filter((t) => !t.startsWith('// This TEAL was generated by TEALScript v'))
    .join('\n');

  return {
    teal,
    b64: Buffer.from(teal).toString('base64'),
  };
}
...
export function artifactsTest(
  ...
  describe(`${className} Artifacts`, () => {
    ...
    test('Generates TEAL', () => {
      expect(getTealWithoutVersion(compiler.teal[target]).teal).toMatchSnapshot();
    });
    ...
    if (!options.lsig) {
      test('Generates ABI JSON', () => {
        expect(compiler.arc4Description()).toMatchSnapshot();
      });

      test('Generates ARC32', () => {
        const arc32 = compiler.arc32Description();
        arc32.source.approval = getTealWithoutVersion(compiler.teal.approval).b64;
        arc32.source.clear = getTealWithoutVersion(compiler.teal.clear).b64;
        expect(arc32).toMatchSnapshot();
      });

      test('Generates ARC56', () => {
        const arc56 = compiler.arc56Description();
        arc56.source!.approval = getTealWithoutVersion(compiler.teal.approval).b64;
        arc56.source!.clear = getTealWithoutVersion(compiler.teal.clear).b64;
      });
    }
  });
}
