...
interface Subroutine extends ABIMethod{
  allows: {create: string[], call: string[]}
}
...
export default class Compiler {
  ...
  async compile() {
    ...
    if (this.subroutines.map((a) => a.allows.create).flat().length === 0) {
      ...
      this.subroutines.push({ ...m, allows: { create: ['NoOp'], call: [] } });
  ...
  private processMethodDefinition(node: ts.MethodDeclaration) {
      ...
      (d) => {
        ...
        switch (decoratorClass) {
          case 'abi':
            ...
            break;
          case 'allow':
            if (!['call', 'create'].includes(decoratorFunction)) throw Error(`Unknown decorator ${d.getText()}`);

            if (decoratorFunction === 'create' && callExpr.arguments.length === 0) {
              ...
              if (!ON_COMPLETES.includes(arg.text)) throw Error(`Invalid OnComplete: ${arg.text}`);
              this.currentSubroutine.allows[decoratorFunction as 'call' | 'create'].push(arg.text);
            }
            break;
    ...
    if (allows.create.length + allows.call.length === 0) {
