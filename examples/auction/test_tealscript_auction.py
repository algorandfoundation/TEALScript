...
def create_app():
    ...
    app_client = client.ApplicationClient(
        app=open("./tealscript_artifacts/Auction.json").read(),
...
def opt_in():
    ...
    sp = app_client.get_suggested_params()
    sp.fee = sp.min_fee * 2
    app_client.call("optIntoAsset", asset=asa, suggested_params=sp)
...
def start_auction():
    ...
    app_client.call("startAuction", axfer=axfer, startingPrice=10_000, length=36_000)
...
def send_first_bid():
    ...
    app_client.call(
        "bid",
        payment=pay_txn,
        previousBidder=first_bidder.address,
        signer=first_bidder.signer,
    )
...
def claim_bid():
    sp = app_client.get_suggested_params()
    sp.fee = sp.min_fee * 2
    app_client.call("claimBid", suggested_params=sp)
...
def claim_asset():
    ...
    app_client.add_method_call(
        ...
        method="claim_asset",
        asset=asa,
        assetCreator=creator_acct.address,
        suggested_params=sp,
        sender=second_bidder.address,
        signer=second_bidder.signer,
    ),
...
def test_create_highest_bidder(create_app):
    assert (
        encode_address(bytes(app_client.get_global_state()["highestBidder"], "utf-8"))
...
def test_create_highest_bid(create_app):
    assert app_client.get_global_state()["highestBid"] == 0


@pytest.mark.create
def test_create_auction_end(create_app):
    assert app_client.get_global_state()["auctionEnd"] == 0
...
def test_start_auction_highest_bid(create_app, opt_in, start_auction):
    assert app_client.get_global_state()["highestBid"] == 10_000
...
def test_first_bid_highest_bid(create_app, opt_in, start_auction, send_first_bid):
    assert app_client.get_global_state()["highestBid"] == 20_000


@pytest.mark.first_bid
def test_first_bid_highest_bidder(create_app, opt_in, start_auction, send_first_bid):
    addr = bytes.fromhex(app_client.get_global_state()["highestBidder"])
...
def test_second_bid_highest_bid(
    ...
    assert app_client.get_global_state()["highestBid"] == 30_000
